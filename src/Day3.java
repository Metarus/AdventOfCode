...
public class Day3 {
    public static void main(String[] args) {
        ...
        for(int i=0; i<wire1instructions.length; i++) {
            ...
            int dist=Integer.parseInt(wire1instructions[i].substring(1));
            System.out.println(dist);
            for(int j=0; j<dist; j++) {
                y+=dy;
                x+=dx;
                steps++;
                wire1over[x][y]=(short)steps;
            }
        }
        x=25000;
        y=25000;
        String[] wire2instructions=wire2.split(",");
        int smallest=999999;
        steps=0;
        for(int i=0; i<wire2instructions.length; i++) {
            dx=0;
            dy=0;
            switch(wire2instructions[i].charAt(0)) {
                case 'R':
                    dx=1;
                    break;
                case 'L':
            ...
            int dist=Integer.parseInt(wire2instructions[i].substring(1));
            for(int j=0; j<dist; j++) {
                y+=dy;
                x+=dx;
                steps++;
                if(wire1over[x][y]!=0) {
                    if(wire1over[x][y]+steps<smallest) smallest=wire1over[x][y]+steps;
                }
                //alternatively, when use a boolean array and check the distance, but i got rid of that while doing part b
            }
        }
        System.out.println();
